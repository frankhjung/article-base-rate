...
COVID-19 hospitalisation data is openly reported but the information provided is
being misrepresented. Take for example the following Facebook post:

![Facebook 24 Jan 2022](images/facebook-2022-01-24.png)

While accurate with regards to the numbers, Peter is implying that vaccination
does not work as more people are in hospital who have been vaccinated than those
that have not. This is a misrepresentation because the emphasis Peter makes
is on the _absolute number_ rather than the rate of hospitalisations among
vaccinated versus unvaccinated individuals.

As an example let's use some data approximately based on the figures
available from the same source as Peter's, namely NSW health.
...
vaccinated_hospitalisation_rate
```

## Understanding the Numbers

The rates reveal a dramatically different picture from the absolute numbers:

- Unvaccinated hospitalisation rate: **`r round(unvaccinated_hospitalisation_rate, 2)`** per 1,000 people
- Vaccinated hospitalisation rate: **`r round(vaccinated_hospitalisation_rate, 2)`** per 1,000 people
- **Unvaccinated people are hospitalised at `r round(unvaccinated_hospitalisation_rate / vaccinated_hospitalisation_rate, 1)` times the rate of vaccinated people**
...
The base rate fallacy occurs when we ignore the base rate (the underlying proportion)
of a population when evaluating statistics. In this case:
...
effective. The key is to compare the **rates** within each group, not the raw counts.
...
cat(
  "Vaccinated death rate:",
  round(vaccinated_death_rate, 3), "per 1,000\n"
)
cat(
  "Unvaccinated death rate:",
  round(unvaccinated_death_rate, 3), "per 1,000\n"
)
cat(
  "Unvaccinated are", round(unvaccinated_death_rate / vaccinated_death_rate, 1),
...
hospitalised at approximately **`r round(unvaccinated_hospitalisation_rate / vaccinated_hospitalisation_rate, 1)` times the rate**
of vaccinated individuals, and die at approximately
**`r round(unvaccinated_death_rate / vaccinated_death_rate, 1)` times the rate**.
...
Focusing solely on absolute numbers, as in the Facebook post cited, leads to
the base rate fallacy and fundamentally misrepresents the protective effect of
vaccination.
